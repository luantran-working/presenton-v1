  ...
  const fetchCustomModels = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setCustomModels([]);
        setCustomModelsChecked(true);
        toast.error("Lỗi khi tải mô hình tùy chỉnh");
      }
    } catch (error) {
    ...
    <div className="space-y-6">
        ...
        <div className="mb-4">
          <button
            ...
            className={`w-full py-2.5 px-4 rounded-lg transition-all duration-200 border-2 ${
              customModelsLoading || !customLlmUrl
      ...
      {customModelsChecked && customModels.length === 0 && (
        <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
          <p className="text-sm text-yellow-800">
            Không tìm thấy mô hình nào. Vui lòng đảm bảo khóa API của bạn hợp lệ
            và có quyền truy cập vào các mô hình.
          </p>
        </div>
      )}
        ...
        <div className="mb-4">
          <div className="mb-3 p-3 bg-amber-50 border border-amber-200 rounded-lg">
            <p className="text-sm text-amber-800">
              <strong>Quan trọng:</strong> Chỉ những mô hình có khả năng gọi hàm
              (tool calls) hoặc hỗ trợ JSON schema mới hoạt động.
            </p>
          </div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
            Select Model
            ...
            <Popover open={openModelSelect} onOpenChange={setOpenModelSelect}>
              ...
              >
                <Command>
                  ...
                  <CommandList>
                    <CommandEmpty>Không tìm thấy model.</CommandEmpty>
                    <CommandGroup>
      ...
      <div>
        ...
        <p className="mt-2 text-sm text-gray-500 flex items-center gap-2">
          ...
          If enabled, Tool Calls will be used instead of JSON Schema for
          Structured Output.
        </p>
      </div>
