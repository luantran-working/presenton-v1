  ...
  const fetchOllamaModels = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setOllamaModels([]);
        toast.error("Lỗi khi tải mô hình Ollama");
      }
    } catch (error) {
          ...
          <div className="mb-4">
            ...
            <div className="relative">
              <input
                type="text"
                required
                placeholder="Nhập URL Ollama của bạn"
            ...
            <p className="mt-2 text-sm text-gray-500 flex items-center gap-2">
              ...
              Thay đổi điều này nếu bạn đang sử dụng instance Ollama tùy chỉnh
            </p>
          </div>
        )}
      </div>
          ...
          {ollamaModelsLoading ? (
            <div className="w-full h-12 px-4 py-4 border border-gray-300 rounded-lg bg-gray-50 flex items-center justify-center">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-gray-600">Đang tải model...</span>
              </div>
            </div>
                ...
                >
                  <div className="flex gap-3 items-center">
                    <span className="text-sm font-medium text-gray-900">
                      {ollamaModel
                        ? ollamaModels?.find((m) => m.value === ollamaModel)
                            ?.label || ollamaModel
                        : "Chọn một model"}
                    </span>
                    {ollamaModel && (
        ...
        {(!ollamaModels || ollamaModels.length === 0) &&
          !ollamaModelsLoading && (
            <p className="mt-2 text-sm text-gray-500">
              Không có mô hình nào khả dụng. Vui lòng kiểm tra kết nối Ollama
              của bạn.
            </p>
          )}
      </div>
