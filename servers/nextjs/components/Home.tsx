...
export default function Home() {
  ...
  const handleSaveConfig = async () => {
    ...
    try {
      ...
      toast.info("Đã lưu cấu hình thành công");
      setButtonState((prev) => ({
        ...prev,
        isLoading: false,
        isDisabled: false,
  ...
  useEffect(() => {
    if (
      ...
      setButtonState({
        isLoading: true,
        isDisabled: true,
        text: `Đang tải model (${percentage}%)`,
        showProgress: true,
    ...
    if (downloadingModel && downloadingModel.done) {
      setTimeout(() => {
        setShowDownloadModal(false);
        setDownloadingModel(null);
        toast.info("Đã tải model thành công!");
      }, 2000);
    }
  }, [downloadingModel]);
      ...
      <main className="flex-1 container mx-auto px-4 max-w-3xl overflow-hidden flex flex-col">
        ...
        <div className="text-center mb-2 mt-4 flex-shrink-0">
          ...
          <p className="text-gray-600 text-sm">
            Trình tạo bài thuyết trình AI mã nguồn mở
          </p>
        </div>
                ...
                downloadingModel.status !== "pulled" && (
                  <div className="text-xs text-gray-500">
                    ...
                    {downloadingModel.status === "verifying" &&
                      "Đang xác minh tính toàn vẹn model..."}
                    {downloadingModel.status === "pulling" &&
                      "Đang kéo model từ registry..."}
                  </div>
                )}
              ...
              {downloadingModel.downloaded && downloadingModel.size && (
                <div className="mt-4 p-3 bg-gray-50 rounded-lg">
                  <div className="flex justify-between text-xs text-gray-600">
                    ...
                    <span>
                      Tổng cộng:{" "}
                      {(downloadingModel.size / 1024 / 1024).toFixed(1)} MB
                    </span>
                  </div>
                </div>
              )}
