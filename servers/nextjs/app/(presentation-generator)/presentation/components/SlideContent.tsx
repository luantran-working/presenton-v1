...
const SlideContent = ({ slide, index, presentationId }: SlideContentProps) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error("Lỗi khi chỉnh sửa slide:", error);
      toast.error("Lỗi khi chỉnh sửa slide.", {
        description: error.message || "Lỗi khi chỉnh sửa slide.",
      });
    } finally {
      setIsUpdating(false);
    }
  ...
  const onDeleteSlide = async () => {
    try {
      ...
      toast.error("Lỗi khi xóa slide.", {
        description: error.message || "Lỗi khi xóa slide.",
      });
    }
  };
        ...
        >
          ...
          {!showNewSlideSelection && (
            <div className="group-hover:opacity-100 hidden md:block opacity-0 transition-opacity my-4 duration-300">
              <ToolTip content="Thêm slide mới bên dưới">
                {!isStreaming && !loading && (
                  <div
                    onClick={() => {
                      trackEvent(
          ...
          {!isStreaming && (
            <div className="absolute top-2 z-20 sm:top-4 hidden md:block left-2 sm:left-4 transition-transform">
              <Popover>
                <PopoverTrigger>
                  <ToolTip content="Cập nhật slide bằng prompt">
                    <div
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <div className=" cursor-pointer ">
                    <ToolTip content="Hiển thị ghi chú diễn giả">
