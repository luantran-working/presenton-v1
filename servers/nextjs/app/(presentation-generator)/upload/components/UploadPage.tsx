...
const UploadPage = () => {
  ...
  const validateConfiguration = (): boolean => {
    if (!config.language || !config.slides) {
      toast.error("Vui lòng chọn số slide và ngôn ngữ");
      return false;
    }

    if (!config.prompt.trim() && files.length === 0) {
      toast.error("Không có lời nhắc hoặc tài liệu được cung cấp");
      return false;
    }
    return true;
  };
  ...
  const handleDocumentProcessing = async () => {
    setLoadingState({
      ...
      duration: 90,
      extra_info:
        files.length > 0 ? "Có thể mất vài phút đối với tài liệu lớn." : "",
    });
  ...
  const handleGenerationError = (error: any) => {
    ...
    toast.error("Lỗi", {
      description: error.message || "Lỗi trong trang upload.",
    });
  };
