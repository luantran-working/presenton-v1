...
const SupportingDoc = ({ files, onFilesChange }: SupportingDocProps) => {
  ...
  const filesWithIds: FileWithId[] = files.map((file) => {
    ...
    fileWithId.id = `${file.name || "unnamed"}-${
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (invalidFiles.length > 0) {
      toast.error("Loại file không hợp lệ", {
        description: "Vui lòng chỉ tải lên file PDF, TXT, PPTX hoặc DOCX",
      });
      return;
    }
    ...
    if (
      hasPdf &&
      droppedFiles.some((file) => file.type === "application/pdf")
    ) {
      toast.error("Không được phép sử dụng nhiều file PDF", {
        description: "Vui lòng chỉ chọn một file PDF",
      });
      return;
    }
    ...
    if (validFiles.length > 0) {
      ...
      toast.success("Đã chọn file", {
        description: `${validFiles.length} file đã được thêm`,
      });
    }
  };
  ...
  const removeFile = (fileId: string) => {
    const updatedFiles = files.filter((file) => {
      const currentFileId = `${file.name || "unnamed"}-${
        file.lastModified || Date.now()
      }-${file.size || 0}`;
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6">
          ...
          <p className="text-gray-600 text-center mb-2">
            {isDragging
              ...
              : "Kéo và thả file của bạn vào đây hoặc nhấp vào nút bên dưới"}
          </p>
          <p className="text-gray-400 text-sm text-center mb-4">
            Hỗ trợ file PDF, Text, PPTX, DOCX
          </p>
              ...
              >
                {filesWithIds.map((file) => {
                  return (
                    ...
                    >
                      <div
                        className="p-4 bg-purple-50 group-hover:bg-purple-100 
